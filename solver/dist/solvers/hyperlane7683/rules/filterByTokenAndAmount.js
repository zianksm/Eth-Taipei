...
import z from "zod";
const FilterByTokenAndAmountArgs = z.record(z.string(), z.record(z.string(), z.union([
    z.null(),
    z.bigint().refine((max) => max > 0n, { message: "Invalid Max Amount" }),
])));
export function filterByTokenAndAmount(args) {
    ...
    return async (parsedArgs) => {
        const tokenIn = bytes32ToAddress(parsedArgs.resolvedOrder.minReceived[0].token);
...
//# sourceMappingURL=filterByTokenAndAmount.js.map