...
export async function withdrawRewards(intent, originChainName, multiProvider, protocolName) {
    log.info({
        msg: "Settling Intent",
    ...
    await new Promise((resolve) => prover.once(prover.filters.IntentProven(_hash, claimantAddress), async () => {
        log.debug(`${protocolName} - Intent proven: ${_hash}`);
        const settler = IntentSource__factory.connect(metadata.intentSources.find((source) => source.chainName == originChainName).address, signer);
        const tx = await settler.withdrawRewards(_hash);
        const receipt = await tx.wait();
        const baseUrl = multiProvider.getChainMetadata(originChainName).blockExplorers?.[0]
            .url;
